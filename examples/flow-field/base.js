...
export async function evolve(generation) {
	...
	try {
		...
		spinner.start(`Evolving ${generation} -> ${generation + 1} | ${spawns} spawns left`);
		...
		spinner.succeed(`Evolving ${generation} -> ${generation + 1} | ${spawns} spawns left`);
		await import(`./${nextFilename}`);
	} catch (error) {
		spawns--;
		spinner.fail(`Evolving ${generation} -> ${generation + 1} | ${spawns} spawns left`);
